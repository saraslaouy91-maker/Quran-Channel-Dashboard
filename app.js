...
(function () {
  ...
  /* طبقة أساس محلية بالكامل (GeoJSON) بديلة عن بلاطات الخرائط الخارجية —
     تعمل دون اتصال بالإنترنت. الألوان تتبع متغيّرات السمة (فاتح/داكن). */
  const geoStyle = () => ({
    color: cvar("--line-2"), weight: 0.7, opacity: 1,
    fillColor: cvar("--surface-2"), fillOpacity: 1
  });
  function initMap() {
    ...
    if (window.WORLD_GEOJSON) {
      geoLayer = L.geoJSON(window.WORLD_GEOJSON, { style: geoStyle, interactive: false }).addTo(map);
    }
    map.attributionControl.setPrefix(false);
    map.attributionControl.addAttribution("خريطة أساس محلية · Natural Earth");
    drawMarkers();
  }
  ...
  function refreshMapTheme() { if (!map) return; if (geoLayer) geoLayer.setStyle(geoStyle()); drawMarkers(); }
